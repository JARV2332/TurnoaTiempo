...
import { isAppControlSearchParams, withAppControl } from '@/lib/app-control'
import { EncargadoAppHome } from '@/components/encargado/encargado-app-home'
...
export default async function EncargadoDashboardPage({
  ...
  const appControl = isAppControlSearchParams(params)
  ...
  if (!user) {
    const login = appControl ? '/auth/login?app=control' : '/auth/login'
    redirect(login)
  }
  ...
  if (appControl) {
    return (
      <EncargadoAppHome
        procesionActiva={procesionActiva}
        procesionesProgram={procesionesProgram}
      />
    )
  }

  const link = (href: string) => withAppControl(href, false)
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gestiona las procesiones de tu hermandad</p>
        </div>
        <Button asChild>
          <Link href={link('/encargado/procesiones/nueva')}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex gap-2">
                <Button asChild variant="outline">
                  <Link href={link(`/encargado/procesiones/${procesionActiva.id}`)}>
                ...
                <CardContent>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" asChild className="flex-1">
                      <Link href={link(`/encargado/procesiones/${procesion.id}`)}>
                        <Settings className="mr-2 h-4 w-4" />
                        Configurar
                      </Link>
                    </Button>
            ...
            {procesionesFinalizadas.slice(0, 6).map((procesion) => (
              <Card key={procesion.id} className="glass-card opacity-75">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base">{procesion.nombre}</CardTitle>
                  <CardDescription>
                    {procesion.fecha ? formatearFechaISO(procesion.fecha) : 'Sin fecha'}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button variant="ghost" size="sm" asChild className="w-full">
                    <Link href={link(`/encargado/procesiones/${procesion.id}`)}>Ver detalles</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
      ...
      {(!procesiones || procesiones.length === 0) && (
        <Card className="glass-card">
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <h3 className="text-lg font-medium mb-1">No hay procesiones</h3>
            <p className="text-sm text-muted-foreground mb-4">Crea tu primera procesión para comenzar</p>
            <Button asChild>
              <Link href={link('/encargado/procesiones/nueva')}>
                <Plus className="mr-2 h-4 w-4" />
                Nueva Procesión
