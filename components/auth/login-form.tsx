...
import { isAppControlSearchParams, persistAppControlSession } from '@/lib/app-control'
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.role === 'superadmin' && !appControl) {
        ...
        router.push(appControl ? '/encargado?app=control' : '/encargado')
      }
    } catch (err: unknown) {
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-balance text-center">
              {appControl ? 'Turno a Tiempo — Control' : 'Turno en Tiempo Real'}
            </h1>
            <p className="text-sm text-muted-foreground text-center">
              {appControl ? 'App de campo para el cortejo' : 'Panel de Gestión de Procesiones'}
            </p>
          </div>
